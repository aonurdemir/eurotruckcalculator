...
public class MaxTruck {
    ...
    public int solve(int target, int truckCount, int day, int cash, int creditToPay, List<String> state) {
        ...
        if (cash >= truckPrice) {
            state.add("BuyWithCash");
            d2 = solve(target, truckCount + 1, day + 1, cash - truckPrice + dailyIncome, creditToPay,  state);
            state.remove(state.size() - 1);
        } else if (creditToPay == 0 && cash + creditAmount >= truckPrice) {
            int netCash = cash + creditAmount - truckPrice;
            state.add("BuyWithCredit");
            d3 = solve(target, truckCount + 1, day + 1, netCash + dailyIncome, creditToPay + creditAmount, state);
            state.remove(state.size()-1);
        }else{
            //no action
            state.add("NA-");
            d1 = solve(target, truckCount, day + 1, cash + dailyIncome, creditToPay,  state);
            state.remove(state.size() - 1);
        }
    ...
    public static void main(String[] args) {
        MaxTruck s = new MaxTruck();
        int days = s.solve(20, 5, 0, 40, 0, new ArrayList<>());
